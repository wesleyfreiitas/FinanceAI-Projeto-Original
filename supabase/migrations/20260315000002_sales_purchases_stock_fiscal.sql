-- ============================================================
-- Sales Orders (Pedidos de Venda / Orçamentos)
-- ============================================================
CREATE TABLE public.sales_orders (
  ...
  -- Extras
  payment_method TEXT,
  payment_terms INTEGER,          -- prazo em dias
  notes TEXT,
  internal_notes TEXT,
  -- Comissão
  salesperson TEXT,
...
CREATE POLICY "cm_soi_select" ON public.sales_order_items FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.sales_orders o WHERE o.id = order_id AND is_company_member(o.company_id)));
CREATE POLICY "cm_soi_insert" ON public.sales_order_items FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.sales_orders o WHERE o.id = order_id AND is_company_member(o.company_id)));
CREATE POLICY "cm_soi_update" ON public.sales_order_items FOR UPDATE
  USING (EXISTS (SELECT 1 FROM public.sales_orders o WHERE o.id = order_id AND is_company_member(o.company_id)));
CREATE POLICY "cm_soi_delete" ON public.sales_order_items FOR DELETE
  USING (EXISTS (SELECT 1 FROM public.sales_orders o WHERE o.id = order_id AND is_company_member(o.company_id)));
...
CREATE POLICY "cm_poi_select" ON public.purchase_order_items FOR SELECT
  USING (EXISTS (SELECT 1 FROM public.purchase_orders o WHERE o.id = order_id AND is_company_member(o.company_id)));
CREATE POLICY "cm_poi_insert" ON public.purchase_order_items FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM public.purchase_orders o WHERE o.id = order_id AND is_company_member(o.company_id)));
CREATE POLICY "cm_poi_update" ON public.purchase_order_items FOR UPDATE
  USING (EXISTS (SELECT 1 FROM public.purchase_orders o WHERE o.id = order_id AND is_company_member(o.company_id)));
CREATE POLICY "cm_poi_delete" ON public.purchase_order_items FOR DELETE
  USING (EXISTS (SELECT 1 FROM public.purchase_orders o WHERE o.id = order_id AND is_company_member(o.company_id)));
...
-- ============================================================
-- Invoices / Notas Fiscais (placeholder)
-- ============================================================
